...
-(void)GetImageResource:(NSString*)sessionKey ImageId:(NSString*)imageId SizeType:(NSString*)sizeType{
    NSURL* url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/streaming/api/v1/%@/image/%@/%@", _host, sessionKey, imageId, sizeType]];
    self.runningTask = [self.urlSession dataTaskWithURL:url];
...
//            });
    ...
    if( _serviceCall == GetImageResourceCall){
        ...
        BYTE* imageData = _memBuffer->EmptyBufferGetPtr();
        dispatch_async(dispatch_get_main_queue(), ^(){
            [_delegate OnImageResourceResponse:[NSData dataWithBytesNoCopy:imageData length:imageDataSize] MimeType:mimeType];
            delete imageData;
        });
    }
